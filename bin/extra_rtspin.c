/* A slightly modified rtspin that increases its wcet after a given 
 * execution time has elapsed. This is useful for experiments where 
 * task-set is schedulable in the beginning and becomes unschedulable
 * after a task exhibits an increase in its wcet.  
 */
...
static void usage(char *error) {
	...
	fprintf(stderr,
		"Usage:\n"
		"	rt_spin [COMMON-OPTS] WCET PERIOD DURATION EXTRA_WCET EXTRA_WCET_DURATIO\n"
...
int main(int argc, char** argv)
{
	...
	lt_t period;
	lt_t extra_wcet;
	double wcet_ms, period_ms, extra_wcet_ms;
	int migrate = 0;
	int cpu = 0;
	int opt;
	int wait = 0;
	...
	double duration = 0, extra_wcet_duration = 0, start;
	...
	extra_wcet_ms = atof(argv[optind + 3]);
	extra_wcet_duration = atof(argv[optind + 4]);
	
	extra_wcet = extra_wcet_ms * __NS_PER_MS;
	...
	if (file) {
		...
		start = wctime();
		while (job(extra_wcet_ms * 0.001 * scale, start + extra_wcet_duration));
	}
